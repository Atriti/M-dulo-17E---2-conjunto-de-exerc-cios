...
@app.route("/resultado_contacto", methods=["POST"])
def resultado_contacto():
...
def edit_user_atual(id):
    ...
    db = get_db()
    db.execute("UPDATE users SET username=?, email=?, perfil=? WHERE id=?", (username, email, perfil, id))
    db.commit()
    return redirect("/users")
...
def delete_user(id):
    ...
    db.commit()
    session["msg"] = "Usuário eliminado com sucesso!" 
    return redirect("/users")
...
@app.route("/edit_perfil/<int:id>", methods=["POST"])
@login_required
def atualizar_perfil(id):
    ...
    password = request.form.get("password")
    db = get_db()
    db.execute("UPDATE users SET username=?, email=?, password=? WHERE id=?", (username, email, password, id))
    db.commit()
